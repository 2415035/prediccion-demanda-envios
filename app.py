...
# Configuración para manejar UTF-8 en la salida
import sys
...
# Asegurar que las columnas de texto estén en UTF-8
for df in [envios, regiones, eventos, tipo_servicio, rutas]:
    df.applymap(lambda x: str(x).encode('utf-8').decode('utf-8') if isinstance(x, str) else x)
